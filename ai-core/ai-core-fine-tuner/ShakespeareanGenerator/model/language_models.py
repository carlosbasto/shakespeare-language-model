...
class ShakespeareanLanguagelModel(nn.Module):
    ...
    def resize_token_embeddings(self, new_num_tokens):
        old_num_tokens = self.embeddings.num_embeddings
        self.embeddings = nn.Embedding(new_num_tokens, self.embeddings.embedding_dim)
        self.output = nn.Linear(self.embeddings.embedding_dim, new_num_tokens)
        if new_num_tokens > old_num_tokens:
            self.embeddings.weight.data[:old_num_tokens].copy_(self.embeddings.weight.data[:old_num_tokens])
        else:
            self.embeddings.weight.data.copy_(self.embeddings.weight.data[:new_num_tokens])
