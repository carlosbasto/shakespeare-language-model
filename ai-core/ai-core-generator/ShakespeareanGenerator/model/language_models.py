...
tracking = Tracking()
logging = Logger()
serving_params = ServingParameters()

class Head(nn.Module):

    def __init__(self, head_size):
        super().__init__()
        self.key = nn.Linear(serving_params.embedding_dim, head_size, bias=False)
        self.query = nn.Linear(serving_params.embedding_dim, head_size, bias=False)
        self.value = nn.Linear(serving_params.embedding_dim, head_size, bias=False)
        self.register_buffer('tril', torch.tril(torch.ones(serving_params.context_length, serving_params.context_length)))

        self.dropout = nn.Dropout(serving_params.dropout)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        self.projection = nn.Linear(head_size * num_heads, serving_params.embedding_dim)
        self.dropout = nn.Dropout(serving_params.dropout)
...
class FeedForward(nn.Module):
    ...
    def __init__(self, embedding_dim):
        ...
        self.ffnet = nn.Sequential(
            ...
            nn.Dropout(serving_params.dropout)
        )
...
class ShakespeareanLanguagelModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.embeddings = nn.Embedding(serving_params.dictionary_size, serving_params.embedding_dim)
        self.position_embeddings = nn.Embedding(serving_params.context_length, serving_params.embedding_dim)
        self.transformer_blocks = nn.Sequential(
            *[TransformerBlock(serving_params.embedding_dim, num_heads=serving_params.attention_heads) for _ in range(serving_params.num_layers)]
        )
        self.layer_norm = nn.LayerNorm(serving_params.embedding_dim)
        self.output = nn.Linear(serving_params.embedding_dim, serving_params.dictionary_size)
...
class ModelTrainer:
    ...
    def __init__(self, data_handler, model):
        ...
        self.optimizer = torch.optim.AdamW(
            self.model.parameters(), lr=serving_params.learning_rate
        )

    def train(self):
        try:
            for iteration in range(serving_params.iteration_limit):
                if iteration % serving_params.eval_frequency == 0 or iteration == serving_params.iteration_limit - 1:
