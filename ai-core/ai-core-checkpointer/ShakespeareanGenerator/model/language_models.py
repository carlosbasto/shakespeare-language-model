...
class ModelTrainer:
    ...
    def train(self):
        ...
        for iteration in range(training_params.iteration_limit):
            ...
            logging.info('Sent to Data Handler for Tokenization and Generating Batches for iteration {}'
                          .format(iteration))
            logits, loss = self.model(batches_x, batches_y)
            logging.info('Forward Pass for iteration {}'
                          .format(iteration))

            self.optimizer.zero_grad(set_to_none=True)
            loss.backward()
            logging.info('Backward Pass for iteration {}'
                          .format(iteration))
            self.optimizer.step()
            logging.info('Optimization Step for iteration {}'
                          .format(iteration))
